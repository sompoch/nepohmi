...
class getOpcItem(object):
    ...
    def add_child_tag(self,iter,treestore,list,type_tag):
        ...
        if type_tag == 'OPC':
            ...
            get_from_db = TagDatabase(None)
            query = "SELECT * FROM HOST"
            lst_server = get_from_db.get_opc_on_database(query)
            print "Load opc server from database ",lst_server
            if lst_server is not None:
                for j in lst_server:
                    host_id = j[0]
                    iter1 = treestore.append(iter, [j[1]])
                    query = "SELECT * FROM OPC_SERVER_NAME WHERE HOST_ID ="+str(host_id)
                    lst_opc_name = get_from_db.get_opc_on_database(query)
                    for h in lst_opc_name:
                        opc_name = h[2]
                        iter2 = treestore.append(iter1, [opc_name])
                        #------#Load tag item -----------------------
                        #get opc server id 
                        query = "SELECT Id FROM OPC_SERVER_NAME WHERE OPC_NAME=\'"+opc_name+"\' AND HOST_ID="+str(host_id)+";"
                        print query
                        opc_server_id = get_from_db.get_opc_on_database(query)
                        if len(opc_server_id)>0:
                            print "OPC SERVER ID = ",opc_server_id[0][0]
                
                            #GET ALL ITEM 
                            query = "SELECT ITEM FROM TAG WHERE HOST_ID="+str(host_id)+" AND OPC_NAME_ID ="+str(opc_server_id[0][0])+";"
                            print query
                            
                            _item  = get_from_db.get_opc_on_database(query)
                            if _item is not None :
                                result = []
                                for t in _item:
                                    result.append(t[0])
                                self.loadOPCtoTreeView(iter2,None,treestore,result)
    ...
    def add_device_manual(self,widget,treeview, treestore):
        ...
        if selected_parent == 'OPC':
            if iter:
                ...
                add_result =  self.add_dialog_opc(treeview,init_,"Add")
                if add_result is not None:
                    print "Add result",add_result
                    con_db = TagDatabase(None)
                    #Add OPC Host Name IP Address and Port To Database
                    add_host = con_db.add_host(add_result[0],add_result[1],add_result[2],"Add",None)
    ...
    def add_dialog_opc(self,treeview,defualt,flag):
        #------------------------------------------------
        #Dialog for Add and Edit OPC SERVER NAME IP PORT 
        #You can use this function to add and edit server 
        # def add_dialog_opc(self,treeview,default,flag)
        # treeview = object ,   treeview widget
        # default  = dictionary ,{} type dict for input data 
        # flag     = string ,["Add","Update"] state to add 
        #            or edit opc server use "Add" type = String 
        #            for another is "Update"
        #------------------------------------------------
        global result
        result = None
        ...
        def check_exist(server_name):
            con_db = TagDatabase(None)
            return_serv = con_db.tag_exist("HOST","SERVER_NAME",server_name)
            print "Check Server name exist = ",return_serv
            serv_exist = False
            if return_serv is not None:
                serv_exist = True
                self.warning_dialog("Server Name \'% s \' has exist!\r\n Try again!" % (entry.get_text()))
            return serv_exist
        ...
        def ip_key_press(self, event):
            #-----------------------------------------
            #Exit dialog by press ENTER from key board
            #-----------------------------------------
            global result
            keyname = gtk.gdk.keyval_name(event.keyval)
            if keyname == 'Return':
                print "Close by Enter Key"
                # Call check Server has exist in database 
                # You can add same Ip adderss and port but not same opc server name
                if flag == "Add":
                    exist = check_exist(entry.get_text())
                    if exist == False:
                        #Retrun tuple data
        ...
        port_entry.show()
        port_entry.set_text(str(defualt['port']))
        ...
        print "Set dialog to center screen"
            ...
            if iter is not None:
                ...
                if parent_item is not None:
                    ...
                    if parent_item is not None:
                        ...
                        if root_parent == 'OPC':
                            print "Found  OPC "
                            self.popupMenu(event,'click_opc_name',widget, treestore)
    ...
    def deleteDevice(self,widget,treeview, treestore):
        ...
        if result: #result could be None
            ...
            del_server = TagDatabase(None)
            
            query = "SELECT Id FROM HOST WHERE SERVER_NAME=\'"+selected_item+"\';"
            print query
            host_id = del_server.get_opc_on_database(query)

            #Delete OPC_SERVER_NAME TABLE
            if host_id is not None:
                query = "DELETE FROM OPC_SERVER_NAME WHERE HOST_ID=\'"+str(host_id[0][0])+"\';"
                print query
                result  = del_server.delete_tag(query,False,str(host_id[0][0])) #False == Popup dialog confirm before delete
                
                if result !="Cancel Delete":
                    query = "DELETE FROM HOST WHERE SERVER_NAME=\'"+selected_item+"\';"
                    print query
                    result  = del_server.delete_tag(query,True,str(host_id[0][0])) #True == bypass Popup dialog confirm before delete
                    tree_model.remove(iter)
                    self.buttonApply.set_sensitive(True)
    ...
    def dialogProperty(self,widget,treeview, treestore):
        ...
        if result: #result could be None
            ...
            if selected_parent == 'OPC':
                con_db = TagDatabase(None)
                info  = con_db.get_host_name("HOST","SERVER_NAME",selected_item)
                if info is not None:
                    MESSAGE = 'OPC Server Name :'+ info[0] + '\r\n'
                    MESSAGE = MESSAGE + 'IP Address   : ' + info[1] +'\r\n'
                    MESSAGE = MESSAGE + 'Connection Port  : ' + str(info[2]) +'\r\n'
            ...
            if selected_parent not in ['OPC','Local','Bacnet IP']:
                #Check OPC SERVER NAME from database 
                con_db = TagDatabase(None)
                query = "SELECT * FROM HOST WHERE SERVER_NAME=\'"+selected_parent+"\';"
                print query
                get_server_name = con_db.get_opc_on_database(query) # return list inner tuple
                if len(get_server_name) >0:
                    host_id = get_server_name[0][0]
                    host_name = get_server_name[0][1]
                    host_ip = get_server_name[0][2]
                    host_port = get_server_name[0][3]
                    
                    MESSAGE = 'OPC Server Name \t:  '+ selected_item + '\r\n'
                    MESSAGE = MESSAGE + 'IP Address        \t:  ' + host_ip +'\r\n'
                    MESSAGE = MESSAGE + 'Connection Port  \t:  ' + str(host_port) +'\r\n'
                    #
                    try:
                        result= pyroweb_client.opc_info(host_ip,host_port,selected_item)
                        if result is not None:
                            MESSAGE = MESSAGE + "------------------------------------------------\r\n"
                            for info in result:
                                head = "%-20s %s " % (info[0],"\t:  "+info[1])
                                MESSAGE = MESSAGE +head+"\r\n"
                                print info
                        #MESSAGE =  MESSAGE + str(result)
                    except Exception,err:
                        MESSAGE = MESSAGE + "\r\n<span color = 'red'> Error found : "+str(err)+"</span>"
                else:
                    
                    MESSAGE = "None"
                '''tmp = []
                for t in get_server_name:
                    tmp.append(t[0])
                if selected_parent in tmp:
                    print "You get opoc server property from ",selected_parent
                    query = "SELECT Id FROM HOST WHERE SERVER_NAME"
                    get_server_name = con_db.get_opc_on_database(query)'''
    ...
    def edit_opcserver(self,widget,treeview, treestore):
        ...
        if selected_parent == 'OPC':
            if iter:
                ...
                #Connect database "tag.db"
                con_db = TagDatabase(None)
                edit_result = con_db.get_host_name("HOST","SERVER_NAME",selected_item)
                #Return value to edit_result = SERVER_NAME,IP,PORT
                if edit_result is not None:
                    defualt['opc server name'] = edit_result[0]
                    defualt['ip'] = edit_result[1]
                    defualt['port'] = edit_result[2]
                    edit_result =  self.add_dialog_opc(treeview,defualt,"Edit")
                    print "Edit result ",edit_result
                    if edit_result is not None:
                        con_db = TagDatabase(None)
                        #Edit OPC Host Name IP Address and Port To New Value
                        add_host = con_db.add_host(edit_result[0],edit_result[1],edit_result[2],"Edit",defualt['opc server name'])
                        tree_model.set_value(iter, 0, edit_result[0])
                else:
                    self.warning_dialog("Can't found  \'%s\' property in database " % (selected_item))

                '''edit_result =  self.add_dialog_opc(treeview,defualt)
                print "Add OPC Server Result = ",edit_result
                if add_result is not None:
    ...
    def loadOPCtoTreeView(self,iter1,treeview,treestore,device_tag):


        #Remove iter before load new
        if treeview is not None:
            ...
            if result: #result could be None
                tree_model, iter = result
                item_iter = tree_model.iter_children(iter1)
                #for iter_c in file_iter:
        ...
        print "Type device tag ",type(device_tag)
        device_tag.sort()
        for h in device_tag:
            if h != 'info' :
    ...
    def popupMenu(self,event,state,treeview, treestore):
        ...
        if state == 'click_opc_name':
            browe = gtk.MenuItem("Browe...")
            browe.connect("activate",self.get_opc_item,treeview, treestore)
            browe.show()
            menu.append(browe)
            
            remove = gtk.MenuItem("Remove")
            remove.connect("activate",self.remove_opc_server,treeview, treestore)
            remove.show()
            menu.append(remove)
    ...
    def get_opc_item(self,widget,treeview, treestore):
        selection = treeview.get_selection()
        result = selection.get_selected()
        MESSAGE = "None"
        #print "delete selection ",result
        if result: #result could be None
            ...
            if len(get_server_name) >0:
                ...
                try:
                    result= pyroweb_client.opc_item(host_ip,host_port,selected_item)
                    if result is not None:
                        MESSAGE = MESSAGE + "------------------------------------------------\r\n"
                        #for tag in result:
                            #print tag
                        ...
                        con_db = TagDatabase(None)
                        add_tag = con_db.add_tag_to_db(host_id,selected_item,result,None)
                        
                        #TODO : load item to treeview
                        self.loadOPCtoTreeView(iter,treeview,treestore,result)
                        self.buttonApply.set_sensitive(True)
    ...
    def remove_opc_server(self,widget,treeview, treestore):
        #label = gtk.Label("\r\n\r\nServer name and property")
        #label.show()
        ...
        if result: #result could be None
            tree_model, iter = result
            selected_server = tree_model.get_value(iter, 0)
            
            print 'selected server from list',selected_server
            parent_item = tree_model.iter_parent(iter)
            selected_host = tree_model.get_value(parent_item, 0)
            print 'selected server parent from list = ',selected_host
            
            con_db = TagDatabase(None)
            query = "SELECT Id FROM HOST WHERE SERVER_NAME=\'"+selected_host+"\';"
            print query
            result_id = con_db.get_opc_on_database(query) # return list inner tuple
            if len(result_id) > 0 : #Make sure data return correct
                host_id = result_id[0][0]#get host id
                if host_id is not None:
                    
                    #get opc server id 
                    query = "SELECT Id FROM OPC_SERVER_NAME WHERE OPC_NAME=\'"+selected_server+"\' AND HOST_ID="+str(host_id)+";"
                    print query
                    opc_server_id = con_db.get_opc_on_database(query)
                    
                    if opc_server_id is not None:
                        print "OPC SERVER ID = ",opc_server_id[0][0]
                        
                        query = "DELETE FROM TAG WHERE HOST_ID=\'"+str(host_id)+"\' AND OPC_NAME_ID =\'" +str(opc_server_id[0][0])+"\';"
                        print query
                        result  = con_db.delete_tag(query,False,selected_server) #False == Popup dialog confirm before delete
                        
                        if result !="Cancel Delete":
                            
                            query = "DELETE FROM OPC_SERVER_NAME WHERE HOST_ID=\'"+str(host_id)+"\' AND OPC_NAME=\'" +selected_server+"\';"
                            print query
                            result  = con_db.delete_tag(query,True,selected_server) #False == Popup dialog confirm before delete
                            
                        
                            #query = "DELETE FROM HOST WHERE SERVER_NAME=\'"+selected_server+"\';"
                            #print query
                            #result  = del_server.delete_tag(query,True,str(host_id[0][0])) #True == bypass Popup dialog confirm before delete
                            tree_model.remove(iter)
                            self.buttonApply.set_sensitive(True)
    ...
    def read_opc_all(self,widget,tree_view,treestore):
        ...
        con_db  = TagDatabase(None)
        read_server = con_db.get_host_name("HOST","SERVER_NAME",selected_item)
        if read_server is not None:
            ...
            if str(type(result)) == '<type \'str\'>':
                ...
                select_server = self.popup_opc_list(result,waiting_confirm)
                
                
                host_name = selected_item # select item from treeview there is thw computer host name
                
                
                con_db = TagDatabase(None)
                add_opc = con_db.add_opc_to_host(host_name,select_server)
                
                
                if select_server is not None:
                    for g in select_server:
                        iter1 = treestore.append(iter, [g])
                        print g
                        #add_opc_to_host(self,host,list_opc)
...
class TagDatabase():
    ...
    def __init__(self,server_list):
        print "Connect Tag Database With SQLite3"
        self.server_list = server_list
        
    def add_host(self,host,ip,port,flag,option):
        con = None
        ...
        try:
            ...
            with con:
                ...
                if file_exist == False:
                    ...
                    cur.execute("CREATE TABLE HOST(ID INTEGER PRIMARY KEY,SERVER_NAME TEXT,IP TEXT,PORT INT);")
                print "Add host % s ip %s port %s " % (host,ip,port)
                if flag == "Add":
                    sql = "INSERT INTO HOST(SERVER_NAME,IP,PORT) VALUES (\'"+host+"\',\'"+ip+"\',"+str(port)+");"
                    cur.execute(sql)
                else: # Another to update 
                    sql = "UPDATE HOST SET SERVER_NAME=?,IP=?,PORT=? WHERE SERVER_NAME=\'"+option+"\'"
                    print sql
                    cur.execute(sql,(host,ip,port))
                con.commit()
    ...
    def add_tag_to_db(self,host_id,opc_name,tag_list,option):
        if tag_list is not None:
            ...
            print query
            cur.execute(query)
            opc_server_id = cur.fetchone()
            print "OPC SERVER ID = ",opc_server_id[0]
            
            #GET ALL ITEM 
            query = "SELECT ITEM FROM TAG WHERE HOST_ID="+str(host_id)+" AND OPC_NAME_ID ="+str(opc_server_id[0])+";"
            print query
            cur.execute(query)
            tag_exist = cur.fetchall()
            print "Tag count exist = ",len(tag_exist)
            tmp = []
            for g in tag_exist:
                tmp.append(g[0])
            for t in tag_list:
                if t in tmp:
                    ...
                    sql = "INSERT OR REPLACE INTO TAG(HOST_ID,OPC_NAME_ID,ITEM) VALUES (\'"+str(host_id)+"\',\'"+str(opc_server_id[0])+"\',\'"+t+"\') ;"
                    cur.execute(sql)
    ...
    def add_opc_to_host(self,host,list_opc):
        con = None
        database_name = "tag.db"
        con = lite.connect(database_name)
        cur = con.cursor()    
        query = "SELECT Id FROM HOST WHERE SERVER_NAME=\'"+host+"\';"
        cur.execute(query)
        host_id = cur.fetchone()
        print "HOST ID ",host_id
        
        query = "SELECT OPC_NAME FROM OPC_SERVER_NAME WHERE HOST_ID=\'"+str(host_id[0])+"\';"
        cur.execute(query)
        Already_Server = cur.fetchall()
       
        for t in Already_Server:
            if t[0] in list_opc:
                list_opc.remove(t[0])
        ...
        if host_id is not None:
            with con:
                for j in list_opc:
                    query = "INSERT INTO OPC_SERVER_NAME(HOST_ID,OPC_NAME) VALUES (\'"+str(host_id[0])+"\',\'"+j+"\');"
                    cur.execute(query)
    ...
    def delete_tag(self,query,flag,massage):
        #Remove OPC Server and OPC Tag From Database 
        allow_delete = False
        result = -4
        if flag == False:
            result = self.confirm_dialog("Do you want to delete \'%s\' from database and all opc tag in root has been remove ?" %(massage))

        if result == -5 and flag==False: #Press O.K. to Confirm
            allow_delete = True
        else:
            allow_delete = False
            
        if allow_delete == True or flag == True:
            try:
                con = lite.connect('tag.db')
                cur = con.cursor()   
    ...
    def get_host_name(self,frm_table,col,tag):
        #-------------------------------------------------
        #Issue : 22.01.2012
        #This function to return  host name from database 
        #You can select value by SQL query
        #Paramete :
        #frm_table = type string , select from database table 
        #col       = type string , select column name 
        #tag       = type string , select tag name 
        #
        #return    = type tuple  , SERVER_NAME,IP,PORT
        #
        #etc : get_host_name("OPC",""SERVER_NAME","localhost")
        #-------------------------------------------------
        ...
        try:
            ...
            query = "SELECT SERVER_NAME,IP,PORT FROM "+frm_table+" WHERE "+col+"=\'"+tag+"\';"
            cur.execute(query)
            result = cur.fetchone()
            ...
            if con:
                con.close()
            #Return tuple value = SERVER_NAME,IP,PORT
            return result
        
    def get_opc_on_database(self,query):
        rows =None
        try:
            con = lite.connect('tag.db')
            cur = con.cursor()    
        ...
        try:
            ...
            with con:
                ...
                if file_exist == False:
                    ...
                    print "create new database table HOST"  
                    cur.execute("CREATE TABLE HOST(Id INTEGER PRIMARY KEY,SERVER_NAME TEXT,PORT INT)")
                    cur.execute("CREATE TABLE OPC_SERVER_NAME(Id INTEGER PRIMARY KEY,HOST_ID INT,OPC_NAME TEXT);")

                #cur.execute("INSERT INTO OPC(Server,port) VALUES ('OPC_TEST',7777)")
                for j in server_list:
                    sql = "INSERT INTO HOST(SERVER_NAME,IP,PORT) VALUES (\'"+str(j)+"\',7777)"
                    print sql
                    cur.execute(sql)
    ...
    def tag_exist(self,frm_table,col,tag):
        rows =None
        try:
            con = lite.connect('tag.db')
            cur = con.cursor()    
            query = "SELECT * FROM "+frm_table+" WHERE "+col+"=\'"+tag+"\';"
            print query
            cur.execute(query)
            rows = cur.fetchone()
